...
async function getBaseWebpackConfig(dir, { buildId , config , compilerType , dev =false , entrypoints , isDevFallback =false , pagesDir , reactProductionProfiling =false , rewrites , originalRewrites , originalRedirects , runWebpackSpan , target =_constants1.COMPILER_NAMES.server , appDir , middlewareMatchers , noMangling =false , jsConfig , resolvedBaseUrl , supportedBrowsers , clientRouterFilters  }) {
    ...
    async function handleExternals(context, request, dependencyType, layer, getResolve) {
        ...
        if (!isLocal) {
            ...
            const notExternalModules = /^(?:private-next-pages\/|next\/(?:dist\/pages\/|(?:app|document|link|image|legacy\/image|constants|dynamic|script|navigation|headers)$)|string-hash|private-next-rsc-action-proxy$)/;
    ...
    const cache = {
        ...
        version: `${"13.2.5-canary.16"}|${configVars}`,
