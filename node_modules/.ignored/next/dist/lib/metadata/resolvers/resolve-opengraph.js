...
function resolveImages(images, metadataBase) {
    ...
    resolvedImages == null ? void 0 : resolvedImages.forEach((item, index, array)=>{
        if ((0, _resolveUrl).isStringOrURL(item)) {
            array[index] = {
                url: metadataBase ? (0, _resolveUrl).resolveUrl(item, metadataBase) : item
            };
        } else {
            // Update image descriptor url
            item.url = metadataBase ? (0, _resolveUrl).resolveUrl(item.url, metadataBase) : item.url;
        }
    });
